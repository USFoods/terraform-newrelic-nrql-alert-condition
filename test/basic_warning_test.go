...
func TestBasicWarningConfiguration(t *testing.T) {
	...
	assert.Equal(t, "Basic Warning Policy", policyOutput["name"])
	assert.Equal(t, policyOutput["id"], moduleOutput["policy_id"])
	assert.Equal(t, "Basic Warning NRQL Alert Condition", moduleOutput["name"])
	...
	assert.Equal(t, "above", strings.ToLower(moduleOutput["warning_operator"]))
	assert.Equal(t, "500", moduleOutput["warning_threshold"])
	assert.Equal(t, "300", moduleOutput["warning_threshold_duration"])
	assert.Equal(t, "all", strings.ToLower(moduleOutput["warning_threshold_occurrences"]))
