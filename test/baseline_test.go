...
func TestBaselineConfiguration(t *testing.T) {
	...
	assert.Equal(t, "Baseline Policy", output["policy_name"])
	assert.Equal(t, output["policy_id"], output["module_policy_id"])
	assert.Equal(t, "Baseline NRQL Alert Condition", output["name"])

	assert.Equal(t, "above", strings.ToLower(output["critical_operator"]))
	assert.Equal(t, "5", output["critical_threshold"])
	...
	assert.Equal(t, "baseline", output["type"])
	assert.Equal(t, "upper_and_lower", output["baseline_direction"])
