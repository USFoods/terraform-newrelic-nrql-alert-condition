...
func TestBasicCriticalConfiguration(t *testing.T) {
	// Construct the terraform options with default retryable errors to handle the most common
	// retryable errors in terraform testing.
	terraformOptions := terraform.WithDefaultRetryableErrors(t, &terraform.Options{
		// Set the path to the Terraform code that will be tested.
		TerraformDir: "../examples/basic_critical",
	...
	assert.Equal(t, "Basic Critical Policy", outputPolicyName)
	// Assert policy id matches condition policy id
	assert.Equal(t, outputPolicyID, outputConditionPolicyID)
	// Assert name is Basic Critical NRQL Condition
	assert.Equal(t, "Basic Critical NRQL Alert Condition", outputName)
