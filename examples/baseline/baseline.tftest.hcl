...
run "verify_condition_configuration" {
  ...
  assert {
    condition     = output.name == "Baseline NRQL Alert Condition"
  ...
  assert {
    condition     = output.type == "baseline"
    error_message = "condition type did not match expected"
  }

  assert {
    condition     = lower(output.baseline_direction) == "upper_and_lower"
  ...
  assert {
    condition     = output.critical_threshold == 5
